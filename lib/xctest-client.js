...
class XCTest extends EventEmitter {
  ...
  * startBootstrap(isDevice) {
    return new Promise((resolve, reject) => {
      ...
      var args = `clean test -project ${XCTestWD.projectPath} -scheme XCTestWDUITests -destination id=${this.device.deviceId} -UseModernBuildSystem=NO XCTESTWD_PORT=${this.proxyPort}`.split(' ');
      ...
      proc.stdout.on('data', data => {
        ...
        if (xctestrun) {
          ...
          if (match) {
            const url = match[1];
            if (url.startsWith('http://')) {
              logger.debug('hit xcode 13 mode');
              this.configUrl(url);
              resolve();
            }
          }
      ...
      proc.stderr.on('data', data => {
        ...
        if (!xctestrun) {
          if (~data.indexOf(simulatorLogFlag)) {
            ...
            // https://github.com/macacajs/XCTestWD/issues/191
            resolve('');
