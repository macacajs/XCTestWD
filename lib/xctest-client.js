...
class XCTest extends EventEmitter {
  ...
  *startDeviceLog() {
    var proc = childProcess.spawn(devicelog.binPath, [this.device.deviceId], {});
    this.deviceLogProc = proc;
  ...
  startBootstrap() {
    ...
    var args = `test -project ${XCTestWD.projectPath} -scheme ${XCTestWD.schemeName} -destination id=${this.device.deviceId} XCTESTWD_PORT=${this.proxyPort}`.split(' ');
